...
\hypertarget{appendix-b-api-surface}{%
\section{Appendix B. The user-facing surface of the resolution
model}\label{appendix-b-api-surface}}

The classification of §4.1 and the table-local boundary of §5 are not
internal bookkeeping; they are Gazelle\textquotesingle s programming
model. This appendix exhibits the small API surface through which a user
states resolution policies and supplies runtime decisions. The purpose
is to show that the paper\textquotesingle s constructs are directly
programmable, not to survey the generator\textquotesingle s interface:
typed semantic actions, tree construction, and error recovery are
engineering outside this paper\textquotesingle s scope and are
documented with the implementation.

\hypertarget{b1-terminal-kinds}{%
\subsection{B.1 Terminal kinds declare the resolution
policy}\label{b1-terminal-kinds}}
...
conflicted cells of the canonical machine. \texttt{shift} and
\texttt{reduce} are applied at construction and yield ordinary table
entries that participate in completion like any other. \texttt{prec} and
\texttt{conflict} produce the deferred shift-or-reduce cells of §5,
represented through the real/virtual branch labels and protected by the
completion guard described there. A plain terminal involved in a
conflict fails generation, and the report cites canonical contexts, so a
diagnosis like §2.1\textquotesingle s migration example points at the
state where the conflict arises rather than at a merged artifact. A
grammar with counted, intentional plain conflicts may declare totals
(\texttt{expect\ 19\ sr;}), mirroring Yacc practice.

\hypertarget{b2-token-data}{%
\subsection{B.2 Deferred decisions arrive as token
data}\label{b2-token-data}}

A \texttt{prec} or \texttt{conflict} terminal\textquotesingle s payload
carries the resolution datum, produced by the lexer together with the
token:
...
This is everything that crosses §5\textquotesingle s boundary. For
\texttt{prec}, the parser compares the token\textquotesingle s level and
associativity against the pending construct\textquotesingle s
precedence, re-evaluating after every reduction inside a single
\texttt{push} --- the \texttt{1\ +\ 2\ *\ 3\ *\ 4} schedule of §5. For
\texttt{conflict}, the token carries one fixed \texttt{Shift} or
\texttt{Reduce} for that push. Because operators are ordinary data, a
precedence table can be extended while parsing; user-defined operators
require no grammar change and no table regeneration.

\hypertarget{b3-push-loop}{%
\subsection{B.3 The push loop and where context
lives}\label{b3-push-loop}}
...
Push-mode LR parsing itself is standard --- Bison offers
\texttt{\%define\ api.push-pull} --- so the exhibit here is not the loop
but the division of labor it makes explicit. The lexer, which naturally
owns left context, computes the token and its resolution datum; this
generalizes the \texttt{IDENT}/\texttt{KEYWORD} priority device of §4.1
from choosing a token kind to choosing a parse action. The parser then
selects among the actions of the current cell using only that datum and
the bounded stack metadata of §5. History-dependence is thereby located
in token production, where context is legitimately available, rather
than in action selection, which Appendix A.4 shows cannot soundly
reconstruct context erased by a merged table. Semantic actions may in
turn update state that the lexer reads on the next iteration --- the
classical C \texttt{typedef} feedback --- without widening the
resolver\textquotesingle s boundary: whatever the lexer learns still
reaches the parser only as the next token and its datum.
