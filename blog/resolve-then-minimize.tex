...
\urlstyle{same}
\hypersetup{
  pdftitle={Resolve, Complete, Then Minimize: Conflict-Preserving LR Tables as Automata},
  pdfauthor={Gerben Stavenga},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Resolve, Complete, Then Minimize: Conflict-Preserving LR Tables as Automata}
\author{Gerben Stavenga\\
\texttt{stavenga@gmail.com}}
\date{}
...
Many useful LR grammars are intentionally conflicting. A single
expression rule such as \texttt{expr = expr OP expr \textbar{} atom} is
often clearer than a hierarchy of precedence nonterminals, and the
dangling-else grammar is naturally stated with a shift policy. In such
grammars, conflict resolution is part of the language semantics: it
selects the parse tree. Practical generators nevertheless merge states
before resolving conflicts because canonical LR(1) tables can be large.
The merge can create conflicts, or move a real conflict and its
resolution into a context where the canonical parser had an
unconditional action. This paper asks how to compact an LR table without
changing where its semantic choices occur or how they are decided.
...
nondeterminism before context is discarded. Gazelle resolves or
explicitly defers this output nondeterminism in the canonical machine. A
conservative completion then aligns reductions across same-core states,
and ordinary partition refinement computes the quotient. Completion may
...
bytes. The result is a direct route to a compact,
canonical-LR-equivalent parser that preserves intentional conflict
semantics; on the bare grammars evaluated here, its state count is the
same as IELR.
...
LR conflicts are often presented as defects to eliminate from a grammar.
In practice, many are deliberate. The compact expression grammar
\texttt{expr = expr OP expr \textbar{} atom} exposes the operator
structure directly and delegates precedence and associativity to an
explicit policy. The canonical dangling-else grammar similarly states
the syntactic ambiguity plainly and delegates the choice to a shift
policy. Refactoring either grammar into a conflict-free hierarchy is
possible, but can make the grammar longer, less direct, and less useful
to clients that want to choose among the actions dynamically.

For an intentional conflict, resolution is not diagnostic cleanup. It is
a semantic decision that selects a parse tree. A correct compaction must
therefore preserve more than the accepted language: it must not create,
erase, relocate, or change the actions of a canonical conflict. This is
the central problem of the paper.

Canonical LR(1) supplies the reference behavior. Its states retain the
complete one-token right context derived from the grammar, so every
action---and every conflict between actions---occurs in precisely the
context that produced it. Resolving or explicitly deferring conflicts in
this automaton fixes the semantic choices before any contexts are
combined.

The engineering pressure to combine them is size. The canonical
automaton for the SQLite grammar used in this paper contains 66,539 item
sets; its IELR automaton has 1,340 states.

Size here is a question about how many states the machine has, not about
how the eventual action/goto matrix is packed into bytes. Sparse-row
...
state equivalence. This paper concerns the state-count objective.

The two-orders-of-magnitude gap explains the usual construction order:
build or approximate the small machine first, then perform conflict
analysis on it. For conflict-free grammars, compatibility-based merging
can often recover much of this reduction directly. The difficult case is
the one programmers commonly want: a grammar that already contains
meaningful conflicts. Merging LR(1) contexts can create additional
conflicts, and it can also change how a real conflict is resolved.
...
This paper makes four contributions centered on conflict fidelity:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  It gives an item-NFA encoding in which every candidate LR action,
  including reductions, is represented in one labeled transition
  system.
\item
  It identifies LR conflicts with output nondeterminism remaining after
  transition nondeterminism has been eliminated, so conflicts can be
  resolved or deliberately deferred in their canonical contexts.
\item
  It gives an alignment-driven completion that replaces only selected
  error entries with unanimous same-core reductions, exposing common
  LALR-style equivalences to generic partition refinement while
  preserving resolved valid-input behavior.
\item
  ...
  minimization, and uses runtime precedence as a case study in why the
  canonical action set at each conflicted cell must survive compaction.
\end{enumerate}
...
Those conflicts need not indicate a poorly designed grammar. Expression
precedence and dangling else are canonical cases in which the
conflicting production structure is simpler and more legible than its
conflict-free expansion. A precedence, associativity, or shift
declaration supplies the missing semantic choice. The minimization
problem studied here is therefore not how to eliminate such conflicts,
but how to preserve their exact action sets and resolution contexts while
merging everything else that is behaviorally indistinguishable.

\hypertarget{21-how-merging-changes-conflicts}{%
\subsection{2.1 How merging changes
conflicts}\label{21-how-merging-changes-conflicts}}
...
\hypertarget{5-runtime-precedence-as-a-conflict-case-study}{%
\section{5. Runtime precedence as a conflict case
study}\label{5-runtime-precedence-as-a-conflict-case-study}}

Expression parsing makes the paper's premise concrete. A rule such as
...
leaves associativity and precedence to a language policy while keeping
the grammar compact and close to its conceptual structure. Traditional
...
runtime. This works only if compaction preserves the canonical question:
runtime policy must choose among actions that genuinely co-occurred in
that canonical cell, never among actions brought together by a merge.
...
Intentional conflicts are not defects left over after grammar design.
They are often the clearest way to state a language: expression
precedence and dangling else are familiar examples. Their resolution is
part of the grammar's semantics. The hard part of minimizing an LR state
machine is therefore not merely deciding which states look similar, but
preserving where those conflicts occur, which actions they contain, and
how they are decided while context is discarded. Merge-first
constructions must predict or repair that interaction. Gazelle instead
changes both the order of operations and the representation on which
they operate. Packing the resulting table compactly in memory remains
...
It is a direct construction of a conflict-preserving,
canonical-LR-equivalent parser whose state counts, on all six
modifier-stripped grammars evaluated here, equal those produced by IELR.
Production grammars with additional resolution semantics may retain more
states, and equal state counts do not imply equal encoded bytes. More
broadly, the construction illustrates a useful engineering principle:
when a domain-specific object nearly fits a mature generic algorithm,
changing the representation may be more effective than inventing
another domain-specific algorithm.
...
\hypertarget{appendix-b-exposing-lr-as-a-library}{%
\section{Appendix B. Exposing LR as a
library}\label{appendix-b-exposing-lr-as-a-library}}

The paper's direct contribution is the table construction. A secondary
design lesson is that the capability of an LR generator is most useful
when exposed as a library rather than delivered as a framework. The
algorithms make subtle decisions about context, conflicts, recovery, and
runtime feedback; applications should be able to use those decisions
without surrendering control of their build, semantic representation,
lexer interaction, or parse loop.

Here ``library'' means composable ownership boundaries, not merely a
function callable at runtime. Gazelle exposes the grammar, construction,
compiled table, runtime decisions, and parser loop as components. Its
procedural macro is one typed embedding of those components into Rust,
not a separate framework that owns the application. We regard this
combination --- a declarative grammar mapped to Rust enums, ordinary
trait implementations for actions, and a caller-owned loop --- as a
modest secondary design contribution. The appendix exhibits the design;
it does not claim a comparative usability evaluation.

\hypertarget{b1-construction-is-a-library-api}{%
\subsection{B.1 Construction is a library
API}\label{b1-construction-is-a-library-api}}

Table construction is available directly to programs. A tool can parse
or construct a grammar, inspect its diagnostics, compile it, and choose
the runtime interface it needs:

\begin{verbatim}
let grammar = gazelle::parse_grammar(source)?;
let compiled = gazelle::CompiledTable::build(&grammar)?;
let parser = gazelle::CstParser::new(compiled.table());
\end{verbatim}

This path supports grammar analyzers, conflict tools, dynamic grammars,
and alternative semantic stacks. The static interface below invokes the
same construction during Rust compilation and adds typed integration.

\hypertarget{b2-the-grammar-is-only-a-grammar}{%
\subsection{B.2 The grammar is only a
grammar}\label{b2-the-grammar-is-only-a-grammar}}
...
(\texttt{=\textgreater{}\ binop}), not implemented, so the grammar text
stays a grammar: readable on its own, diffable, and reusable across
independent backends. Resolution intent is part of this declaration
rather than a side channel: the terminal modifiers of §4.1
(\texttt{shift}, \texttt{reduce}, \texttt{prec}, \texttt{conflict})
state per terminal how a conflicted cell is decided, a conflict on an
unannotated terminal is a generation error diagnosed against canonical
contexts (§4.1), and counted intentional conflicts are declared with
\texttt{expect} totals in yacc style. Note also what is absent: because
\texttt{prec\ OP} covers a whole precedence family, the grammar needs no
cascade of one nonterminal per precedence level.

\hypertarget{b3-embedding-in-rust}{%
\subsection{B.3 Embedding in Rust}\label{b3-embedding-in-rust}}

\texttt{gazelle!} is a procedural macro: the constructions of §4 run
inside the host compiler, so there is no separate generator invocation
and no generated file to version. For each nonterminal with named
alternatives, the macro emits an enum whose variants mirror those
alternatives and whose fields are typed through an associated-types
trait:
...
Semantic code lives outside the macro as ordinary Rust. In particular,
type errors in semantic actions generally point at the action
implementation, where ordinary editor and compiler tooling applies,
rather than at quoted host-language fragments inside a grammar file.

\hypertarget{b4-actions-as-trait-implementations}{%
\subsection{B.4 Actions as trait
implementations}\label{b4-actions-as-trait-implementations}}

A backend chooses a representation per nonterminal by implementing a
\texttt{Types} trait, and folds nodes by implementing \texttt{Action}
per generated enum:
...
Setting \texttt{type\ Expr\ =\ Box\textless{}calc::Expr\textless{}Self\textgreater{}\textgreater{}}
engages a blanket implementation that materializes the full concrete
tree with no action code at all; setting it to \texttt{f64}, as above,
folds values during parsing and never builds a tree; setting it to
\texttt{Ignore} discards the node, for validation-only parsing. The
choices mix freely across nonterminals, and several backends ---
evaluator, tree builder, pretty-printer --- can implement the same
grammar.

\hypertarget{b5-the-parse-loop-belongs-to-the-caller}{%
\subsection{B.5 The parse loop belongs to the
caller}\label{b5-the-parse-loop-belongs-to-the-caller}}

Generated parsers are push-based; the caller owns the loop:

\begin{verbatim}
while let Some(token) = lexer.next(&actions.ctx)? {
    // Semantic actions may update the state read by the lexer.
    parser = parser.push(token, &mut actions)?;
}
let result = parser.finish(&mut actions)?;
\end{verbatim}

This is where the runtime side of §5 surfaces. A \texttt{prec} or
\texttt{conflict} token carries its resolution datum ---
\texttt{Precedence::Left(6)}, or an explicit
\texttt{Resolution::Shift} or \texttt{Resolution::Reduce} --- and that
datum plus the pending stack precedence is everything that crosses the
table-local boundary. Because the loop is open, semantic actions can
update state that the lexer reads on the next iteration (the classical C
\texttt{typedef} feedback) without widening that boundary: whatever the
lexer learns still reaches the parser only as the next token and its
datum.
