...
\hypertarget{appendix-b-using-the-generator}{%
\section{Appendix B. Using the
generator}\label{appendix-b-using-the-generator}}

The body of this paper is about table construction. This appendix
briefly exhibits the surface through which that construction is used,
because the design goal was never the pipeline alone: recurrent pain
points of classical generators --- a separate generation step, semantic
actions interleaved with the grammar, and a parse loop the caller cannot
control --- shaped the API as much as the automata did.

\hypertarget{b1-the-grammar-is-only-a-grammar}{%
\subsection{B.1 The grammar is only a
grammar}\label{b1-the-grammar-is-only-a-grammar}}
...
contexts (§2.1), and counted intentional conflicts are declared with
...
\hypertarget{b2-embedding-in-rust}{%
\subsection{B.2 Embedding in Rust}\label{b2-embedding-in-rust}}
...
and no generated file to version. For each nonterminal the macro emits
an enum whose variants mirror the grammar alternatives and whose fields
are typed through an associated-types trait:
...
\hypertarget{b3-actions-as-trait-implementations}{%
\subsection{B.3 Actions as trait
implementations}\label{b3-actions-as-trait-implementations}}
...
\hypertarget{b4-the-parse-loop-belongs-to-the-caller}{%
\subsection{B.4 The parse loop belongs to the
caller}\label{b4-the-parse-loop-belongs-to-the-caller}}
...
\texttt{Precedence::Left(6)}, or an explicit \texttt{Shift} or
\texttt{Reduce} --- and that datum plus the pending stack precedence is
everything that crosses the table-local boundary. Because the loop is
open, semantic actions can update state that the lexer reads on the next
iteration (the classical C \texttt{typedef} feedback) without widening
that boundary: whatever the lexer learns still reaches the parser only
as the next token and its datum.
